...
describe('x402 Middleware Integration', function () {
    ...
    beforeEach(() => {
        ...
        const testConfig = {
            ...
            flowPayContractAddress: "stellar:session-meter",
    ...
    describe('Protected Routes (No Payment)', function () {
        it('should return 402 for protected route without headers', async function () {
            ...
            expect(res.header['x-flowpay-token']).to.equal('stellar:usdc-sac');
            expect(res.header['x-payment-currency']).to.equal('USDC');
            expect(res.header['x-flowpay-recipient']).to.equal('GSERVICERECIPIENT11111111111111111111111111111111111111');
            expect(res.header['x-flowpay-settlement']).to.equal('soroban-sac');
